...
import { gs, MAX_HP, MAX_SP, MAX_AS } from './gamestate.js';
...
export function updateHUD() {
  document.getElementById('p1-hp-fill').style.transform = `scaleX(${gs.p1.hp / MAX_HP})`;
  document.getElementById('p2-hp-fill').style.transform = `scaleX(${gs.p2.hp / MAX_HP})`;
...
export function showSpecialPopup(text, col) {
  ...
  el.style.color = col === 0x00e5ff ? 'var(--p1)' : 'var(--p2)'; el.style.filter = `drop-shadow(0 0 10px ${col === 0x00e5ff ? 'var(--p1)' : 'var(--p2)'})`;  el.style.opacity = '1';
...
export function spawnDmgNum(player, dmg, col, isSpecial, isBlocked) {
  ...
  el.style.color = isBlocked ? '#88ccff' : (col === 0x00e5ff ? 'var(--p1)' : 'var(--p2)');
